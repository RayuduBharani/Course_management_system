...
export const FetchProfileInfo = createAsyncThunk(
    "instructor/profile",
    async () => {
        const response = await fetch("http://13.233.183.222:8000/instructor/profile/view", {
            method: "GET",
            credentials: "include"
        })
...
export const UpdateInstructorProfileInfo = createAsyncThunk(
    ...
    async (formData : FORMDATA) => {
        const response = await fetch("http://13.233.183.222:8000/instructor/profile/update", {
            method: "PUT",
            credentials: "include",
            headers : {
