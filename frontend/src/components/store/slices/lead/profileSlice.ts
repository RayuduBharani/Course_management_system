...
export const LeadProfileInfo = createAsyncThunk(
    "/lead/dashboard",
    async () => {
        const response = await fetch("https://course-management-system-il4f.onrender.com/lead/profile/info", {
            credentials: "include"
        })
...
export const UpdateLeadProfileInfo = createAsyncThunk(
    ...
    async (formData: IUpdateProfileData) => {
        const response = await fetch("https://course-management-system-il4f.onrender.com/lead/profile/update", {
            credentials: "include",
            method: "PUT",
            headers: {
